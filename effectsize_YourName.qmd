...
To choose the right effect size, you need to take the distribution and variance into account. Go to the PDF of last lab's activities that is in your Individual Google Drive Folder for this class. Look back at your QQ plot from the last lab and remind yourself if the data is normally distributed or not. We also need to see if the variance is equal. Since the standard deviation is the square root of the variance, you can look at your descriptive statistics (also found in the PDF from the last lab) to see if the SD looks similar or not between the two locations.
...
rstatix::wilcox_effsize(
  data = waterQ2,         #put the dataframe name here
  light ~ location,            #dependent variable ~ factor
  paired = FALSE,
...
The effect size is standardized by the amount of variation in the data (the exact equation depends on which effect size you use). The interpretation of the standardized effect size is included in the magnitude column - negligible, small, moderate, or large. This will help you determine if the difference between means or medians is meaningful or not. The 95% confidence intervals tells you how precisely the effect size was estimated. If the 95% confidence interval does not include zero, it suggests that the effect is statistically significant (if conf.low and conf.high are both positive or both negative than that means the 95% CI does not include zero. But if one is positive and the other is negative then that means the 95% CI does include zero).
