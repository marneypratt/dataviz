...
Before you start working, give this file a new name so it doesn't get written over if you accidentally download the GitHub repository a second time. Go to File in the menu and choose "Save as" and then give it the name "dataviz_YourName.qmd" (replace "YourName" with your full name without any spaces)
...
Replace only the blanks in the code below, you don't need to change anything else (note: do not place anything in the empty parentheses in the n() )
...
-   You can also use [Adobe Color](https://color.adobe.com/create/color-accessibility) to find colorblind safe colors (make sure to select "Color Blind Safe" under tools and login with your Smith gmail account)
...
-   Make sure to name your Google Doc file: **Bio131_WaterQResults_YourName** (example: Bio131_WaterQResults_SophiaSmith) and store it in your individual Google Drive folder for this class
...
Rendering is a way to translate a Quarto document into another file type. 
...
-   Before you Render
    ...
    -   You don't have any remaining blanks in the code chunks
     ...
     -   Save this file with the correct file name (make sure this file is named "dataviz_YourName.qmd" replacing "YourName" with your full name)

-   Render this Quarto file (just select the Render menu item at the top of this window)

    -   First try to render to PDF directly using **format: typst** in the YAML at the top of this document

    -   If `typst` isn't working for you, then you can render to HTML (see "renderToHTML.qmd" for the code that goes in the YAML header to render to HTML)
...
-   Please name your PDF file "dataviz_YourName.pdf" (replacing "YourName" with your name without any spaces)

-   Make sure the PDF version of the file is saved somewhere on your computer that you can get to
