...
If the output is hard to read because of a smaller screen, try clicking the "Show in New Window" pop-out icon in the upper right off the output to open the output in its own window.
...
#Histogram  
ggplot(data = waterQ2, 
       aes(y = __, fill=location)) + #place the variable name in the blank after y=
  geom_histogram(binwidth = __,   #adjust the binwidth as needed
                 color = "white", show.legend = FALSE) + 
  facet_grid(location ~ .) +
  ylab("_____") + #replace this line with the appropriate one given above
  xlab("Count") +
