...
You will learn much more about inferential statistics in a full statistics course. In this class, we are just going to discuss effect sizes. An effect size tells us how big the difference is between groups while usually taking into account the variability as well.

To choose the right effect size, you need to take the distribution and variance into account. Look back at your QQ plot and see if the data is normally distributed or not. Look at your descriptive statistics and graph to see if the dispersion (SD, IQR, range, etc) looks similar or not.
