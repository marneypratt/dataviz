...
-   **Normal Distribution, Equal Variance:** If the data is normal, and the two groups have similar dispersion (i.e. similar variance), then you should use "regular" **Cohen's d effect size**

-   **Normal Distribution, Unequal Variance:** If the data is normal, but the two groups have unequal dispersion (i.e. unequal variance), then you should use the **Cohen's d for Welch test effect size**

-   **Non-normal Distribution:** If the data is NOT normal, then you should use the **Wilcoxon effect size** (also note that some statisticians recommend to use the Cohen's d for Welch test effect size if the variance is very different even if the distribution is not normal)
